...
class AjaxHelpers
{
	...
	static public function buttonAction( $module , $access , $id , $setting)
	{
		...
		if($access['is_detail'] ==1) {
			if($setting['view-method'] != 'expand')
			{
				...
				$html .= '<li><a href="'.URL::to($module.'/show/'.$id).'" '.$onclick.'><i class="fa fa-search"></i> '.Lang::get('core.btn_view').'</a></li>';
			}
		}
		if($access['is_edit'] ==1) {
			$onclick = " onclick=\"ajaxViewDetail('#".$module."',this.href); return false; \"" ;
			...
			$html .= '<li><a href="'.URL::to($module.'/add/'.$id).'" '.$onclick.'><i class="fa  fa-edit"></i> '.Lang::get('core.btn_edit').'</a></li>';
		}
		$html .= '</ul></div>';
		return $html;
	}	
	...
	static public function buttonActionCreate( $module  ,$method = 'newpage')
	{
		...
		$html = '
			<a href="'.URL::to($module.'/add').'" class="tips btn btn-xs btn-primary"  title="'.Lang::get('core.btn_create').'" '.$onclick.'>
	...
	static public function htmlExpandGrid()
	{
		...
		return View::make('admin.module.utility.extendgrid');
	}
