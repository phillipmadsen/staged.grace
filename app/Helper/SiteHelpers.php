...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	//	echo '<pre>'; print_r($forms);echo '</pre>';
		...
		{		
		  ...
		  <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'>
			...
			{{ SiteHelpers::activeLang(\''.$form['label'].'\', (isset($fields[\''.$form['field'].'\'][\'language\'])? $fields[\''.$form['field'].'\'][\'language\'] : array())) }} '.$required.'</label>
			<div class="col-md-6">
	...
	public static function formShow( $type , $field , $required ,$option = array()){
		...
		switch($type)
		{
			...
			case 'text_date';
				$form = "
				<div class=\"input-group m-b\" style=\"width:50%;\">
					{{ Form::text('{$field}', \$row['{$field}'],array('class'=>'form-control date', {$mandatory})) }}
			...
			case 'text_datetime';
				$form = "
				<div class=\"input-group m-b\" style=\"width:50%;\">
					{{ Form::text('{$field}', \$row['{$field}'],array('class'=>'form-control datetime', {$mandatory})) }}
				<span class=\"input-group-addon\"><i class=\"fa fa-calendar\"></i></span></div> ";
				break;				
	...
	public static  function transForm( $field, $forms = array(),$bulk=false , $value ='')
	{
			...
			case 'text_date';	
				$form = "
					<div class=\"input-group m-b\" style=\" margin-bottom:0px;\">
						<input  type='text' name='$field{$bulk}' class='date form-control input-sm'  $mandatory value='{$value}'/>
					<span class=\"input-group-addon\"><i class=\"fa fa-calendar\"></i></span></div> ";
				break;
		...
		$form .='<span class="" data-form-type="'.$type.'"></span>';
		return $form;	
	}
	...
	public static function showUploadedFile($file,$path , $width = 50) {
		...
		if(count($checkfile) >1)
		{
			...
			foreach ($checkfile as $key ) {
				$keys .= '<a href="'.URL::to($path_file . $key).'" target="_blank"> '.$key.' </a><br />';			
			}
			return $keys;
	...
	public static function gridDisplays($val , $field, $arr , $attribute = array()) {
	...
	public static  function currNumber( $num){
		return "<strong>USD</strong> ".number_format($num, 2, '.', ' ');
	}
	
	
	public static function gridDisplay($val , $field, $arr , $attribute = array()) {
