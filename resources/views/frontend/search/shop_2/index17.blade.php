...
<div class="container">
      <div class="row">
        ...
        {{-- <h5>Product {{ $products->currentPage() }} of {!! $products->lastPage() !!}</h5> --}}
        <hr>

          @foreach ($products as $product)
            <div class="col-sm-4">
                <div class="row">
                    <a href="{!! url('/'. getLang(). '/shop/' . $product->slug) !!}">
                    <img class="col-sm-12" src="http://placehold.it/400?text=product+image" alt="{!! $product->slug !!}-image">
                    </a>
                    <a href="{!! url('/'. getLang(). '/shop/' . $product->slug) !!}">{!! $product->name !!}</a>

                  <em>{{ (new Carbon\Carbon($product->published_at))->format('M jS Y g:ia') }}</em>
                    <div class="row">
